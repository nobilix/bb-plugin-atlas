...
Каждый сгенерированный файл, поля манифеста и правило зависимостей описаны в [Package anatomy](/ru/package/).
...
Экспорт по умолчанию — это фабрика из [How a plugin works](/ru/architecture/). Она регистрирует четыре метода RPC и одну команду CLI, и оба вызывают одни и те же хелперы поверх `bb.storage.kv`. Каждая запись заканчивается вызовом `bb.realtime.publish`, который сообщает всем открытым страницам об изменении списка. Контракт экспортируется, чтобы `app.tsx` мог импортировать его тип.
...
Сохраните файл. `bb plugin dev` выведет `reloaded hello`, а `bb hello count` напечатает число. Теперь `bb plugin logs hello` показывает `loaded` от нового экземпляра перед `disposed` от старого: это порядок перезагрузки из [How a plugin works](/ru/architecture/).
...
- Чтобы решить, где ваша идея появится в окне или в агенте, прочитайте [Choosing a surface](/ru/choose/).
...
- Чтобы задать пользователю вопрос из CLI или инструмента, изучите [`bb.ui`](/ru/backend/ui/), слот [`pendingInteraction`](/ru/frontend/pendingInteraction/) и паттерн полного доверия (propose-and-confirm) в [Trust model](/ru/trust/).