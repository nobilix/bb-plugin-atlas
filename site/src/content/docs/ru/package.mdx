...
|---|---|---|---|
| `name` | да | npm-идентификатор и единственный источник id плагина | соглашение требует формат `bb-plugin-<x>`; пакеты со скоупом `@acme/bb-plugin-<x>` тоже работают; id всех плагинов из поставки bb зарезервированы |
...
| `bb.name` | да | человекочитаемое имя в настройках, магазине и боковой панели | это не `name` и не id плагина |
...
Есть одно исключение. Плагин-провайдер держит `@get-bb/plugin-sdk` в `dependencies`, потому что хостовая сборка бандлит подпуть `/provider-bridge` из своей копии плагина.
