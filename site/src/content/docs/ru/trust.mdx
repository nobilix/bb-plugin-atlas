...
| Общая база `bb.db` | в `createPluginApi` передаётся то же соединение `DbConnection`; `kv` и `settings` изолируются по `pluginId`, но это соглашение хелперов, а не строгая граница | `plugin-runtime.ts`, `plugin-api.ts` |
...
Это верно для API `bb.storage`. Но это не строгая граница изоляции, и главным источником правды остаётся `docs/configuration.md`.
:::
...
{/* permalink: docs/configuration.md */}
По умолчанию сервер слушает `127.0.0.1`. Проверка origin на `/api/v1` защищает браузеры от CSRF, но не заменяет аутентификацию: локальный клиент без браузера и заголовка `Origin` спокойно её проходит.
...
- Иконки `bb.branding.experimental_icons` проходят через SVG-валидатор, который работает только на отклонение (reject-only). Если иконка не проходит проверку, загрузка плагина прерывается с ошибкой, указывающей на проблемный файл.
