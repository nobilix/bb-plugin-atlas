...
`harness.lifecycle` предоставляет методы `reload(factory)` и `dispose()`. Сам `harness` наследует inspection, behavior и lifecycle, поэтому `harness.callRpc(...)` и `harness.behavior.callRpc(...)` — одно и то же.
...
Хук `useComposerView()` всегда возвращает `layout: "expanded"` и `run: { isRunning: false, isSubmitting: false }`. `experimental_useSidebarThreadSplit` всегда возвращает `isAvailable: true` и `layout: null`. `experimental_useSidebarThreadPullRequest` никогда не отдаёт `isLoading: true`. В харнессе композера нет конвейера отправки, поэтому `experimental_submit` только записывает опции и очищает черновик.
